...
**macOS Agentだけでは代わりになりません。** ルーターが無くても起動し、そのMacの外向き通信をアプリ名つきで表示します。**フロー発生時に捕まえるため、ポーリング間隔による取りこぼしもありません。** ただし**脅威照合と宛先の補強はルーターが観測した接続にしか適用されません。** Agentだけの構成では、通信は一覧に出るのに、**それが危険だと知らせてくれることは一度もありません。** Agentは、ルーターが既に見ているものへ「どのアプリか」を足すためのものであり、ルーターの代わりではありません。

...
try
{
...

    {
        ...
        // so, because nothing compared the two. This reads the Hub's own file.
        {
            ...
            // The log names LAN, loopback and CGNAT destinations instead of
            // calling them unknown (P3-174), from this same table.
            var named = new (string Address, string? Name)[]
            {
                ("10.0.0.1", "LAN"), ("172.16.0.1", "LAN"), ("172.31.255.255", "LAN"), ("192.168.1.1", "LAN"),
                ("169.254.169.254", "LAN"), ("fd12:3456::1", "LAN"), ("fc00::1", "LAN"), ("fe80::1%12", "LAN"),
                ("::ffff:192.168.1.1", "LAN"), ("[fe80::1]", "LAN"),
                ("127.0.0.1", "loopback"), ("127.255.255.254", "loopback"), ("::1", "loopback"),
                ("100.64.0.1", "CGNAT"), ("100.127.255.255", "CGNAT"),
                // Next door is not.
                ("100.63.255.255", null), ("100.128.0.0", null), ("172.32.0.1", null), ("192.169.0.1", null),
                ("8.8.8.8", null), ("2001:4860:4860::8888", null),
                // Reserved, but not a LAN device one needs named.
                ("224.0.0.251", null), ("ff02::fb", null), ("192.0.2.1", null), ("0.0.0.0", null), ("::", null), ("fec0::1", null),
                ("", null), ("not an address", null),
            };
            foreach (var (address, name) in named)
                Assert(PrivateAddress.NetworkName(address) == name,
                    $"{address} is shown as {name ?? "unknown"}, not {PrivateAddress.NetworkName(address) ?? "unknown"}");
            Assert(PrivateAddress.Ranges.Where(range => range.Shown is not null)
                    .Select(range => $"{range.Network}/{range.PrefixLength}={range.Shown}").Order(StringComparer.Ordinal)
                    .SequenceEqual(["10.0.0.0/8=LAN", "100.64.0.0/10=CGNAT", "127.0.0.0/8=loopback", "169.254.0.0/16=LAN",
                        "172.16.0.0/12=LAN", "192.168.0.0/16=LAN", "::1/128=loopback", "fc00::/7=LAN", "fe80::/10=LAN"], StringComparer.Ordinal),
                "exactly the ranges the backlog decided are named, and no others");
            Assert(PrivateAddress.NetworkNames.SequenceEqual(["LAN", "loopback", "CGNAT"])
                   && PrivateAddress.NetworkNames.All(PrivateAddress.IsNetworkName) && !PrivateAddress.IsNetworkName("JP")
                   && !PrivateAddress.IsNetworkName("lan"),
                "the filter lists them in this order, and a country code is never taken for one");

...
The shared observation model, lightweight collector, deduplication, monitoring-mode state machine, and pass-only Network Extension foundation have completed their technical spike. The next stages are the macOS host app, System Extension integration, local storage, and explicit opt-in Hub integration. No release date is committed yet.

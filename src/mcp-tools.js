// MCP tool definitions and the server factory shared by both transports.
//
// Extracted from mcp-server.js (P2-68), which had grown to hold the tools, the
// transport bootstrap, OAuth wiring, rate limiting and the audit trail at once.
// The tool surface is the part that changes most often for product reasons, so
// it benefits most from being readable on its own.
//
// Behaviour is unchanged. In particular registerTool() still refuses to
// register a tool that carries no permission classification, which is what
// keeps the deny-by-default matrix and the tool list from drifting apart.
'use strict';

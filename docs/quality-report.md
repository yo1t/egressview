...
- **Trust anchor stated accurately (this cycle)**: the fingerprint appears in `SECURITY.md`, `trusted-fingerprints.json`, the project website, and a DNS TXT record. Earlier wording called these four independent channels; three of them are built from this repository and would change together under a single account compromise. The documentation now names the DNS record — `_egressview-release.egressview.com`, served under separate credentials — as the comparison that carries the trust, and gives the `dig` command to fetch it. Telling a user to cross-check two copies of the same source is worse than telling them nothing, because it reads as verification.
...
- **Read-only public demo (PR #182, #184)**: the demo enforces write protection through a separate `DEMO_READ_ONLY` flag, and authenticates visitors as a fixed anonymous `viewer` so there is no login screen and no credential to publish. Enabling that requires `DEMO_MODE` as well, since demo mode is what guarantees this is not a real deployment — it refuses to start under `NODE_ENV=production`, uses a separate database, and disables router collection. All 53 state-changing routes live under `/api`, which is where the middleware mounts; the only two public write routes are the two on its allowlist; and Socket.IO registers no inbound event handlers, so there is no WebSocket bypass. The middleware fails closed — anything not matching the allowlist exactly is rejected.
...
- **Low, demo exposure**: the public demo authenticates every visitor as an anonymous `viewer`, so all synthetic demo data is readable by anyone — which is the point of a demo. No credential is published: enabling anonymous access requires both `DEMO_MODE` and `DEMO_READ_ONLY`, and the internal admin token is randomised at every start under that combination. Writes are refused by the viewer permission set and again by the read-only middleware.

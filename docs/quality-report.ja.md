...
- **Low・エコシステム**: 本番イメージは変更のたびにCIがビルドし起動確認していますが、**実運用で動かした実績はありません。**HTTP表面にはpermission matrixから生成した機械可読な契約ができたが、**記述しているのはアクセス面であってペイロードではない**——リクエスト／レスポンスのボディは依然コード中のZodスキーマにしか書かれていない。
...
減点は、既定のハードウェア連携CIがないこと、OpenAPI契約がアクセス面のみでリクエスト／レスポンスのボディを覆っていないこと、サポート対象のservice成果物を実運用で動かした実績が無いことによります。
...
改善リストの残りは種類として変わりません — 継続fuzzing、OpenAPI契約へのペイロードスキーマ追加。いずれもリリースのblockerではありません。provenanceは作られテストもされていますが、**まだ実際のリリースに乗っていません。**

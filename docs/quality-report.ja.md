...
- **Low・サプライチェーン**: **リリース工程は手作業で、3回連続で飛ばされました。** v2.0.2は署名して公開済みですが、v2.0.0とv2.0.1は資産なしのままで、**次のリリースが同じように出ていくのを妨げるものはまだありません。** エージェントの`.pkg`はAppleによりnotarize・stapleされており、これは実在し独立に検証できる署名ですが、プロジェクト自身のチェックサムやdetached signatureは付いていません。**署名資産を欠くリリースをCIで落とすgateが明らかな対策であり、それは存在しません。**
...
**本レポート初版でこの項目を2点としたのは、直近3リリースが何も持っていなかったためです。** いま8点なのは、それが**是正されたから**であって、評価を甘くしたからではありません。**工程は手作業のままで、再び飛ばされてもCIは止めません** — 残存リスクを参照。
...
**残っているのは「署名が無かったこと」ではなく「無かった理由」です。** 署名は人が覚えていなければならない手順であり、**3回連続で飛んだという事実は、覚えていることが管理策ではないことの証拠です。** パイプラインは一度も失敗していません。失敗したのはその周りの規律です。**署名資産を持たないリリースを「公開済み」と扱わないCI gate**があれば、これは「覚えておくこと」から「忘れられないこと」に変わります。そのgateはまだありません。

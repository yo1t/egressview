...
改善リストの残りは種類として変わりません — 利用者がエクスポートできる診断、Signed-Releasesの残点を埋めるSLSA provenance、継続fuzzing、OpenAPI契約、サポート対象のservice成果物。いずれもリリースのblockerではありません。このリストで唯一「単なる追加改善」ではないのが署名済みリリースであり、それは**既に動く手順を実行するかどうかの問題**です。

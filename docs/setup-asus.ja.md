...
Widemap で使用するための ASUS WiFi アクセスポイントの準備手順です。

Widemap は ASUS デバイスを**WiFi アクセスポイント（AP モードまたは AiMesh ノード）**として使用します。ルーターとしてではありません。Yamaha RTX がすべてのルーティングと NAT を担当し、ASUS AP は L2 の可視性を提供します：どのデバイスが接続中か、どの帯域（2.4G/5G/6G）で、信号強度とトラフィック量はどのくらいか。
...
Widemap は SHA256 チャレンジレスポンスで自動認証し、数秒ごとにクライアント情報を取得します。

...
- `POST /api/backup/prune` accepts `{ "execute": false }` for a fast dry-run or `{ "execute": true }` for confirmed cleanup and returns `202` with a worker job. Planning reads only each file's 100-byte SQLite header, excludes unusable files from the retention count, and protects the newest two usable normal generations plus one usable migration generation. If one class is short, another usable class fills the three-point floor. Only one cleanup job may run; another request receives `409`.
- `GET /api/backup/prune/:jobId` returns job status (`running`, `cancelling`, `timing_out`, `completed`, `cancelled`, `timed_out`, or `failed`), progress, and the completed plan/result. `DELETE /api/backup/prune/:jobId` requests safe cancellation. Execution recomputes the plan and rechecks each candidate's logical size, allocated size, modification time, and header state before deletion. Temporary files are never candidates. Full integrity checks remain mandatory when backups are created and before restore.
...
- `GET /readyz` is an unauthenticated, cache-disabled readiness check. It returns `503` with `{ "status": "not_ready" }` until configuration and DB bootstrap complete, then `200` with `{ "status": "ready" }`. A required audit-store write failure changes this to a detail-bounded `503` `degraded` response until writing recovers. It exposes no router, database path, error text, or credential details.
...
- Every generation path reserves a durable UTC-day budget before provider work. The default principal/provider request and recorded-token limits are documented in `.env.example`; exhausted limits return `429`, and a failed provider call still consumes one request reservation.

...
Reason codes: `unauthorized`, `invalid_token`, `insufficient_scope`, `bad_request`, `not_found`, `method_not_allowed`, `payload_too_large`, `client_error`, `global_rate_limit`, `subject_rate_limit`, `client_rate_limit`, `concurrency_limit`, `request_timeout`, `server_error`. Request-level rows also record the bounded MCP method and HTTP status; tool arguments and response bodies are never stored. A run of failures is the signal to investigate.

...
この手順より前に公開され、現在の検証器を通過できないリリースは、互換性のため旧名を維持した `release-signing/unsigned-releases.json` に種別と理由付きで記録してあります。未署名版に加え、署名はあるものの現在のパッケージ方針が拒否する拡張属性を含む旧版も対象です。ゲートは永久に失敗するのではなく既知の事実として報告します。**常に失敗するゲートは、人が無視することを学ぶゲートです。** 方針の発効日以降に公開されたリリースの登録はテストが拒否するため、**このリストが新しい失敗を黙らせる抜け道になることはありません。**

### 二度と公開してはいけないAgentの版

Agentは `publish-agent-release.js` で `dl.egressview.com` へ公開するもので、GitHubリリースではなく、プラットフォームごとに独自の版番号の系列を持ちます。ビルドまたは公開されたが二度と使ってはいけない版は、`release-signing/withdrawn-agent-releases.json` に**後継と理由を添えて**記録します。

`publish-agent-release.js` はこのリストを読み、dry runでも本番でも公開を拒否します。**人の記憶の中にしかない取り下げは、取り下げではありません。** Windowsの `0.1.117` は、どんな大きさのDBでも終わらない移行を含んだまま6時間配信されていましたが、**それを再び公開することを止めるものはリポジトリの中に何もありませんでした。**

各項目は必ず後継を指します。「これを使うな」は、移る先が無ければ指示の半分でしかないからです。後継が自分より古い場合、あるいは後継自身が取り下げられている場合は、テストが拒否します。テストは公開スクリプトを**読むのではなく実行します** —— 最初に書いた版はソースを拒否メッセージで grep するだけで、検査を `null` に置き換えても**探していた文字列はすべて残っていたため通ってしまいました。**

...
Backup cleanupのpreviewと実行は専用worker threadで行います。計画時に読むのは認識した各バックアップの固定100バイトのSQLiteヘッダーだけなので、処理時間はDB容量ではなく世代数に比例します。復元不能なファイルは保持世代数に含めず、新しい利用可能な復元点を保護し、削除直前に候補のメタデータとヘッダー状態を再照合します。完全なintegrity checkは作成時と復元前の境界で維持します。
